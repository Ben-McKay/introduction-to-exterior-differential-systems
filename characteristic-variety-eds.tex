...
\end{answer}
In a tableau, \(P\) is generated by the polars, so the linearization is precisely the same tableau, modulo the nonlinearity.
...
\end{answer}
To compute the characteristic variety from a tableau:
\begin{enumerate}
\item
...
\end{example}
\prob{proof.Cartan.Kaehler:nabla.char}{Find the characteristic variety of \(\nabla \times u = u - f\).}
...
\end{pmatrix}.
\]
\prob{rank.1}{Linear algebra: prove that \(q\) has rank \(1\) just when either \(q=0\) or \(q\) vanishes on a plane which contains one of the coordinate axes.}
\begin{answer}{rank.1}%
